...
class OrderStatusTestCase(ServiceTestCase):
    ...
    def test_view_order_statuses(self):
        ...
        data = {
            ...
            'password': self.test_password,
            'filter_params': {
                'customer_ids': [c_id, 'fake'],
            },
        }
        response = self.handle_action('view_order_statuses', data)
        self.assertEqual(0, len(response['order_statuses']))

        c_id_another = 'another customer'
        self.add_balance(self.test_login, self.test_password, c_id_another, self.currency)
        data = {
            'login': self.test_login,
            'password': self.test_password,
            'filter_params': {
                'customer_ids': [c_id, c_id_another],
                'order_ids': [ord_id_ch_offed, ord_id_locked],
            },
        }
        response = self.handle_action('view_order_statuses', data)
        self.assertEqual(4, len(response['order_statuses']))
        ...
        data = {
            ...
            'filter_params': {
                'customer_ids': [c_id, c_id_another],
                'order_ids': [ord_id_ch_offed, ord_id_locked, 'fake'],
            },
        }
        response = self.handle_action('view_order_statuses', data)
        self.assertEqual(6, len(response['order_statuses']))
